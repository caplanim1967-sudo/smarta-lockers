...
async function handleCommunity(path, method, request, env, user, url) {
  ...
  if (path === "/api/messages/send" && method === "POST") {
    try {
      ...
      let savedSettings = {};
      try {
        savedSettings = JSON.parse(settingsRow?.msg_settings_json || "{}");
      } catch (_) {
      }
...
// .wrangler/tmp/bundle-r6eKVL/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-r6eKVL/middleware-loader.entry.ts
